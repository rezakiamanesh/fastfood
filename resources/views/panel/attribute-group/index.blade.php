            ...
            <div class="card">
                    ...
                    <h2>
                        ...
                        <div class="pull-left margin-5">
                            @can('panel.attributeGroup.create')
                                <a href="{{ route('panel.attributeGroup.create')  }}"
                                   class="btn btn-outline-success btn-border-radius">ایجاد گروه ویژگی جدید</a>
                            @endcan
                            @can('panel.attributes.index')
                                <a href="{{ route('panel.attributes.index')  }}"
                                   class="btn btn-outline-warning btn-border-radius">فهرست ویژگی ها</a>
                            @endcan
                ...
                <div class="body">
                    <div class="table-responsive">
                        @if (isset($attributeGroups) && count($attributeGroups) > 0)
                                ...
                                @foreach($attributeGroups as $val)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center"><a
                                                    href="{{ route('panel.attributeGroup.status',$val->id) }}">
                                                {{ \App\Utility\Status::getStatus($val->status) }}
                                            </a></td>
                                        <td class="text-center">

                                            @can('panel.attributeGroup.edit')
                                                <a href="{{ route('panel.attributeGroup.edit',['id' => $val->id]) }}"
                                                   class="btn btn-warning btn-circle waves-effect waves-circle waves-float">
                                                    <i class="material-icons">update</i>
                                                </a>
                                            @endcan

                                            @can('panel.attributeGroup.delete')
                                                <button type="button"
                                            ...
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        <form
                                                                action="{{ route('panel.attributeGroup.delete' , ['id' => $val->id])  }}"
                                                                method="post">
                                                            @csrf
                                                            {{ method_field('DELETE')  }}
                        ...
                        {{--                                    <span style="margin-right: 45%">{!! $attributeGroups->render() !!}</span>--}}
                        {{--                                @endif--}}
                        {{--                            </div>--}}
                        {{--                        </div>--}}
                    </div>
