                ...
                <div class="header">
                    <h2>
                        ...
                        <div class="pull-left margin-5">
                            @can('panel.video.create')
                                <a href="{{ route('panel.video.create')  }}"
                                   class="btn btn-outline-default btn-border-radius"> ایجاد ویدیو جدید </a>
                            @endcan
                        </div>
                    </h2>
                    ...
                    <ul class="header-dropdown m-r--5">
                        <li class="dropdown">
                            ...
                            <ul class="dropdown-menu pull-right">
                                ...
                                @can('panel.video.create')
                                    <li>
                                        <a href="{{ route('panel.video.create')  }}" style="font-size: 10px">
                                            ایجاد ویدیو جدید
                                        </a>
                                    </li>
                                @endcan
                            ...
                            <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                ...
                                @foreach($videos as $val)
                                    <tr>
                                        ...
                                        <td class="text-center">{{ isset($val->categories[0]) && !empty($val->categories[0]) ? $val->categories[0]->title : 'بدون دسته بندی' }}</td>
                                        <td class="text-center"><img class="img-100 img-thumbnail"
                                                                     src="{{ isset($val->image[0]) ? $val->image[0]->url : null }}"
                                                                     alt="{{ $val->title }}"></td>
                                        <td class="text-center">
                                            <a @can('panel.video.create') href="{{ route('panel.video.status' , ['id' => $val->id])  }}" @endcan>
                                        ...
                                        <td class="text-center">
                                            ...
                                            @can('panel.video.edit')
                                                <a href="{{ route('panel.video.edit',['id' => $val->id]) }}"
                                                   class="btn btn-warning btn-circle waves-effect waves-circle waves-float">
                                                    <i class="material-icons">update</i>
                                                </a>
                                            @endcan

                                            @can('panel.video.delete')
                                                <button type="button"
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <img
                                                            ...
                                                            alt="{{ $val->title }}">
                                                        <hr>
                                                        <p> توضیحات : <span> {!! $val->description !!} </span>
                                                            <video width="100%" height="240" controls>
                                                                <source src="{{ asset('storage/'.$val->video) }}"
                                                                        type="video/mp4">
                                                        ...
                                                        <span
                                                            class="label label-warning m-1">دسته بندی : {{ isset($val->categories[0]) && !empty($val->categories[0]) ? $val->categories[0]->title : 'بدون دسته بندی' }}</span>

                                                        <span
                                                            class="label label-default m-1">زمان ویدیو : {{ $val->time }}</span>
                                        ...
                                        <div class="modal fade" id="deleteModel{{$val->id}}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        <form
                                                            action="{{ route('panel.video.delete' , ['id' => $val->id])  }}"
                                                            method="post">
                                                            @csrf
                                                            {{ method_field('DELETE')  }}
...
{{--                                    <span style="margin-right: 45%">{!! $videos->render() !!}</span>--}}
{{--                                @endif--}}
                            </div>
                        </div>
                    </div>
